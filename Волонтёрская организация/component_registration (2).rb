  ...
  module VolunteerScheduler
    module UserExtensions
      ...
      def extract_referral_code
        # This would typically be stored in session during registration
        # Implementation depends on how Decidim handles registration flow
        Rails.cache.read("referral_code_#{id}") || 
        Thread.current[:current_referral_code]
      end
      
      def find_referrer(referral_code)
        profile = Decidim::VolunteerScheduler::VolunteerProfile
                    .joins(:user)
      ...
      def cleanup_volunteer_data
        # This is handled by dependent: :destroy, but we can add custom logic here
        Rails.logger.info "Cleaning up volunteer data for user #{id}"
      end
    end
  end
end

# Include the extension in the User model
